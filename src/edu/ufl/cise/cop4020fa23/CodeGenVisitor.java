            ...
            } else if(binaryExpr.getOp().kind() == Kind.EXP)
            {
                ...
                if(binaryExpr.getOp().kind() == Kind.TIMES){
                    ...
                    binary.append("(ImageOps.binaryImageScalarOp(ImageOps.OP.DIV,").append(binaryExpr.getLeftExpr().visit(this,arg))
                            .append(",").append(binaryExpr.getRightExpr().visit(this,arg)).append("))");
                }else if(binaryExpr.getOp().kind() == Kind.PLUS){ //passes but could be a problem when we have scopes
                    binary.append("(ImageOps.binaryImageScalarOp(ImageOps.OP.PLUS,").append(binaryExpr.getLeftExpr().visit(this,arg))
                            .append(",").append(binaryExpr.getRightExpr().visit(this,arg)).append("))");
                }
                ...
                if(binaryExpr.getOp().kind() == Kind.TIMES){
                    binary.append("(ImageOps.binaryImageImageOp(ImageOps.OP.TIMES, ").append(binaryExpr.getLeftExpr().visit(this,arg))
                            .append(" ,").append(binaryExpr.getRightExpr().visit(this,arg)).append("))");
                    //im1$2=ImageOps.cloneImage((ImageOps.binaryImageScalarOp(ImageOps.OP.DIV,im0$2,factor$1)));
                }else if(binaryExpr.getOp().kind() == Kind.DIV){ //passes but could be a problem when we have scopes
                    binary.append("(ImageOps.binaryImageImageOp(ImageOps.OP.DIV,").append(binaryExpr.getLeftExpr().visit(this,arg))
                            .append(",").append(binaryExpr.getRightExpr().visit(this,arg)).append("))");
                }else if(binaryExpr.getOp().kind() == Kind.PLUS){ //passes but could be a problem when we have scopes
                    binary.append("(ImageOps.binaryImageImageOp(ImageOps.OP.PLUS,").append(binaryExpr.getLeftExpr().visit(this,arg))
                            .append(",").append(binaryExpr.getRightExpr().visit(this,arg)).append("))");
                }
            }
            else {
